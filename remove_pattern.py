...
def visualize_clusters(image_path, k=3):
...
def process_license_plate(image, blur_strength=5):
...
if __name__ == "__main__":
    input_image = "image/image3.jpg"  # License plate image to process
    output_image = "output/plate_cleaned_kmeans.jpg"

    image = cv2.imread(input_image)

    cleaned_plate = process_license_plate(image, blur_strength=5)

    cv2.imwrite(output_image, cleaned_plate)
    cv2.imshow("Cleaned Plate (K-Means)", cleaned_plate)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    # Visualize clusters
    visualize_clusters(input_image, k=3)